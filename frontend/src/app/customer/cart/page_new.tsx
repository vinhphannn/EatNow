...
export default function CartPage() {
  ...
  const loadCart = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Auto-select all restaurants when cart loads
        const restaurantIds = new Set(cart.map((item: CartItem) => item.restaurant.id)) as Set<string>;
  ...
  const selectedItems = cartItems.filter(item => selectedRestaurants.has(item.restaurant.id));
  const subtotal = selectedItems.reduce((sum, item) => sum + item.subtotal, 0);
  const deliveryFee = selectedRestaurants.size * 15000; // 15k per restaurant
                    ...
                    <div className="p-4 border-b bg-gray-50">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <input
                            type="checkbox"
                            checked={selectedRestaurants.has(restaurantId)}
              ...
              <div className="bg-white rounded-lg shadow-sm border p-6 sticky top-4">
                ...
                <div className="space-y-3 mb-6">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{selectedRestaurants.size}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  {selectedRestaurants.size > 1 && (
                    <div className="border-t pt-3 mt-3">
                      ...
                      {Object.entries(groupedCartItems)
                        .filter(([restaurantId]) => selectedRestaurants.has(restaurantId))
                        .map(([restaurantId, group]) => {
                          const groupSubtotal = group.items.reduce((sum, item) => sum + item.subtotal, 0);
