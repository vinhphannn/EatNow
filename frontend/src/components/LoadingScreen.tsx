...
export default function LoadingScreen({ onComplete, duration = 3000 }: LoadingScreenProps) {
  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      update() {
        // Simple particle movement - không có hiệu ứng né chuột
        this.x += this.vx;
        this.y += this.vy;
    ...
    const animate = () => {
      ...
      // Draw connections between nearby particles - mạng lưới đơn giản
      particlesRef.current.forEach((particle, i) => {
        particlesRef.current.slice(i + 1).forEach(otherParticle => {
          ...
          if (distance < 120) { // Khoảng cách kết nối
            ctx.save();
            ctx.globalAlpha = (120 - distance) / 120 * 0.6; // Opacity
            ctx.strokeStyle = '#ffffff';
            ctx.lineWidth = 0.8; // Độ dày
            ctx.beginPath();
